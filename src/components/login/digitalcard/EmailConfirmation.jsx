...
const EmailConfirmation = () => {
    ...
    const onSubmit = (data) => {
        if (data.email_verification_code === correctCode) {
            navigate('/u-r/d-select-categories');
        } else {
            setError('email_verification_code', {
                type: 'manual',
                message: 'Code is incorrect',
                ...
                <div className="md:container mx-auto h-full">
                    <motion.div
                        ...
                        <div className="md:max-w-[480px] mx-auto">
                            <h2 className="woi_log_main_head !text-start mt-5 mb-3">
                                Enter your code
                            </h2>
                            <p className="text-gray-600 mb-5 text-sm md:text-base">
                                We sent a temporary 6-digit code to <span>neenu1@worldemart.com</span>
                            </p>

                            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col w-full gap-4 mt-5 sm:mt-10">
                                <div className="relative w-full flex gap-3">
                                    <input
                                        ...
                                        className={`woi_log_input_01 peer ${getBorderColor()}`}
                                    />

                                    <label htmlFor="email_verification_code" className="woi_label_absolute_01">
                                        Enter code
                                    </label>
                                </div>
                                <div className="flex flex-col gap-0">
                                    <button
                                        type="submit"
                                        className={`woi_log_btn_01 ${codeStatus === 'correct' ? 'woi_log_btn_02' : 'woi_log_btn_03'
                                            }`}
                                    >
                                        Continue
                                    </button>
                                    {watchCode.length === 6 && (
                                        <>
                                            {codeStatus === 'incorrect' && (
                                                <p className="text-red-500 text-xs mt-0">Wrong email or verification code.</p>
                                            )}
                                            {/* {codeStatus === 'correct' && (
